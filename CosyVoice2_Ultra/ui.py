...
#generate{background:linear-gradient(135deg,#7c3aed,#06b6d4);border:0;border-radius:18px;font-size:20px;font-weight:800;min-height:60px}
...
def generate_handler(files, mic, text, language, style, emotion, preset, custom_dict, temperature, top_p, top_k, speed, guidance_scale, cfg_scale, voice_strength, style_strength, creativity, seed, max_chunk_chars, normalize, remove_silence, noise_reduction, export_format):
    ...
    if mic: refs.append(mic)
    wav, metrics, status = ENGINE.generate(text, refs, language, style, emotion, preset, custom_dict, temperature=temperature, top_p=top_p, top_k=top_k, speed=speed, guidance_scale=guidance_scale, cfg_scale=cfg_scale, voice_strength=voice_strength, style_strength=style_strength, creativity=creativity, seed=seed, max_chunk_chars=max_chunk_chars, normalize=normalize, remove_silence=remove_silence, noise_reduction=noise_reduction)
    exported = export_audio(wav, export_format) if wav else None
    return exported, exported, metrics, status, hardware_markdown()
...
def build_ui() -> gr.Blocks:
    ...
    with gr.Blocks(theme=gr.themes.Soft(primary_hue="violet", neutral_hue="slate"), css=CSS, title="CosyVoice2 Ultra Studio") as demo:
        gr.HTML('<div class="title">🎙️ CosyVoice2 Ultra Voice Generation Studio</div><div class="subtitle">Offline professional TTS, long-form narration, reference analysis, mastering, and export.</div>')
        with gr.Row():
        ...
        with gr.Accordion("Advanced Generation & Mastering Controls", open=False):
            with gr.Row():
        ...
        generate.click(generate_handler, [files, mic, text, language, style, emotion, preset, custom_dict, temperature, top_p, top_k, speed, guidance_scale, cfg_scale, voice_strength, style_strength, creativity, seed, max_chunk_chars, normalize, remove_silence, noise_reduction, export_format], [audio, download, metrics, status, hardware])
    return demo
